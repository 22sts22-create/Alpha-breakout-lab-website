...
# ── MailerLite (draft-and-review email alerts) ───────────────────────────────
# Set these as GitHub Secrets, same as the Pushover keys above.
#   MAILERLITE_API_KEY   — account API token (Integrations → Developer API)
#   MAILERLITE_GROUP_ID  — the subscriber group/list to target
#   MAILERLITE_FROM      — sender email (what subscribers see as "from")
#   MAILERLITE_FROM_NAME — sender display name
# If MAILERLITE_API_KEY is blank, the draft step is silently skipped (bot still
# runs + pushes to Pushover as before). The bot NEVER sends — it only creates a
# DRAFT that waits in MailerLite for you to review and send manually.
MAILERLITE_API_KEY   = os.environ.get("MAILERLITE_API_KEY", "")
MAILERLITE_GROUP_ID  = os.environ.get("MAILERLITE_GROUP_ID", "")
MAILERLITE_FROM      = os.environ.get("MAILERLITE_FROM", "alerts@alphabreakoutlab.com")
MAILERLITE_FROM_NAME = os.environ.get("MAILERLITE_FROM_NAME", "Alpha Breakout Lab")
...
def create_mailerlite_draft(new_entries):
    """Create a DRAFT email campaign in MailerLite with the newly-found PDUFA dates.
    Returns (ok, message). NEVER sends — the draft waits in MailerLite for manual
    review and send. Safe no-op if not configured or no new entries."""
    if not MAILERLITE_API_KEY:
        log.info("MailerLite not configured — skipping draft creation")
        return (False, "not configured")
    if not new_entries:
        return (False, "no new entries")
    if not MAILERLITE_GROUP_ID:
        log.warning("MAILERLITE_GROUP_ID not set — cannot target a list; skipping draft")
    ...
    # Build subject + HTML body from the new entries
    n = len(new_entries)
    today = datetime.now().strftime("%b %d, %Y")
    subject = f"{n} new FDA catalyst{'s' if n != 1 else ''} — {today}"

    rows = ""
    for e in new_entries:
        rows += (
            f"<tr>"
            f"<td style='padding:8px 12px;border-bottom:1px solid #1E2640;'>"
            f"<strong style='color:#06B6D4;'>{js_escape(e['ticker'])}</strong></td>"
            f"<td style='padding:8px 12px;border-bottom:1px solid #1E2640;'>{js_escape(e['company'])}</td>"
            f"<td style='padding:8px 12px;border-bottom:1px solid #1E2640;'>{js_escape(e['date'])}</td>"
            f"<td style='padding:8px 12px;border-bottom:1px solid #1E2640;'>{js_escape(e['risk'])}</td>"
            f"</tr>"
        )
    html = (
        f"<h2>New FDA Catalysts Found</h2>"
        f"<p>{n} new PDUFA date{'s' if n != 1 else ''} were just added to the "
        f"<a href='https://alphabreakoutlab.com/pdufa_calendar'>Alpha Breakout Lab calendar</a>.</p>"
        f"<table style='width:100%;border-collapse:collapse;font-family:sans-serif;'>"
        f"<tr><th align='left' style='padding:8px 12px;'>Ticker</th>"
        f"<th align='left' style='padding:8px 12px;'>Company</th>"
        f"<th align='left' style='padding:8px 12px;'>PDUFA Date</th>"
        f"<th align='left' style='padding:8px 12px;'>Risk</th></tr>"
        f"{rows}</table>"
        f"<p style='margin-top:20px;'>[ADD YOUR WEEKLY NOTE HERE BEFORE SENDING]</p>"
        f"<p style='font-size:12px;color:#64748B;'>Dates are auto-detected from public SEC filings "
        f"and may change. Always verify in the company's latest filing. Not financial advice.</p>"
        f"<p style='font-size:12px;color:#64748B;'><a href='{{$unsubscribe}}'>Unsubscribe</a></p>"
    )

    try:
        # 1) Create the campaign (starts in 'draft' status)
        create_body = {
            "name": f"PDUFA Alert {today}",
            "type": "regular",
            "emails": [{
                "subject": subject,
                "from_name": MAILERLITE_FROM_NAME,
                "from": MAILERLITE_FROM,
                "content": html,
            }],
            "groups": [str(MAILERLITE_GROUP_ID)],
        }
        r = requests.post(f"{base}/campaigns", headers=headers, json=create_body, timeout=20)
        if r.status_code not in (200, 201):
            log.error(f"MailerLite create draft failed: {r.status_code} — {r.text[:300]}")
            return (False, f"create failed {r.status_code}")
        camp = r.json().get("data", {})
        camp_id = camp.get("id")
        log.info(f"MailerLite DRAFT created (id={camp_id}, status={camp.get('status')}). "
                 f"Review & send manually in the MailerLite dashboard.")
        return (True, f"draft {camp_id}")
    except Exception as e:
        log.error(f"MailerLite draft creation error: {e}")
        return (False, str(e))


def send_pushover_notification(findings, new_entries_count, errors, ml_draft_result=None):
    ...
    if new_entries_count > 0:
        ...
        # Tell me whether an email draft is waiting for review
        if ml_draft_result is not None:
            ok, msg = ml_draft_result
            if ok:
                message += f"\n\n📧 Email DRAFT created in MailerLite — review & send it."
            else:
                message += f"\n\n📧 Email draft NOT created ({msg}) — check bot logs."
        priority = 1
    else:
...
def main():
    ...
    if new_entries:
        ...
        if entries_added > 0:
            ...
            # Create a DRAFT email campaign (waits for manual review + send)
            ok, msg = create_mailerlite_draft(new_entries)
            ml_draft_result = (ok, msg)
    else:
        log.info("No new entries to add — calendar unchanged")

    send_pushover_notification(all_findings, entries_added, errors, ml_draft_result)
